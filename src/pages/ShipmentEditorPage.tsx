...
import { autoRoute, createBlankRoute, downloadRoute, localDateTime, makeStop, unroutedLegs, validateRoute } from "../lib/route";
...
export function ShipmentEditorPage() {
  ...
  function updateStop(index: number, stop: ShipmentStop) {
    ...
    const stops = [...route.stops]; stops[index] = stop;
  ...
  return (
    <>
      ...
      {tab === "setup" && <section className="editor-stack"><section className="panel section-heading"><div><p className="eyebrow">Stops and geometry</p><h2>Route setup</h2><p className="muted">Edit coordinates directly or calculate rail geometry from the embedded NS network.</p></div><div className="toolbar"><button className="button" disabled={working} onClick={() => void runAutoRoute()}>Auto-route legs</button><button className="button primary" onClick={() => update("stops", [...route.stops, makeStop(route.stops.length, { type: "Destination", coords: route.stops.at(-1)?.coords })])}>+ Add stop</button></div></section>{route.stops.map((stop, index) => <StopEditor key={stop.id} stop={stop} index={index} total={route.stops.length} onChange={(next) => updateStop(index, next)} onRemove={() => removeStop(index)} />)}</section>}
