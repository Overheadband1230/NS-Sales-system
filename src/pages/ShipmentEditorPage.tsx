...
export function ShipmentEditorPage() {
  ...
  return (
    <>
      ...
      {tab === "setup" && <section className="editor-stack"><section className="panel section-heading"><div><p className="eyebrow">Stops and geometry</p><h2>Route setup</h2><p className="muted">Type a city and select it to fill the nearest NS rail coordinates automatically. Then auto-route the legs.</p></div><div className="toolbar"><button className="button" disabled={working} onClick={() => void runAutoRoute()}>Auto-route legs</button><button className="button primary" onClick={() => update("stops", [...route.stops, makeStop(route.stops.length, { name: "", type: "Destination", coords: [Number.NaN, Number.NaN] })])}>+ Add stop</button></div></section>{route.stops.map((stop, index) => <StopEditor key={stop.id} stop={stop} index={index} total={route.stops.length} onChange={(next) => updateStop(index, next)} onRemove={() => removeStop(index)} />)}</section>}
